...
def get_race_result(jcd: str, date_str: str, race_no: int) -> dict:
    ...
    try:
        ...
        res["order"] = sorted(res["order"], key=lambda x: x["rank"])[:6]
        # 重複除去
        seen_ranks = set()
        unique_order = []
        for o in res["order"]:
            if o["rank"] not in seen_ranks:
                seen_ranks.add(o["rank"])
                unique_order.append(o)
        res["order"] = unique_order
        ...
        for tbl in soup.find_all("table"):
            ...
            for bt in bet_types:
                ...
                for m in re.finditer(pat, tbl_text):
                    ...
                    key = f"{bt}_{combo}"
                    if key not in seen_payouts and amount_str.isdigit():
        ...
        if not res["payouts"]:
            for bt in bet_types:
                pat = rf'{bt}[|\s]+(\d[\d\-＝=]+\d)[|\s]+([\d,]+)円'
                for m in re.finditer(pat, full):
                    combo = m.group(1).replace("＝", "=")
                    amt = m.group(2).replace(",", "")
                    key = f"{bt}_{combo}"
                    if key not in seen_payouts and amt.isdigit():
                        seen_payouts.add(key)
                        res["payouts"].append({
...
def generate_scenario(scored_racers, weather, venue_jcd):
    ...
    ws = weather.get("wind_speed", 0)
    wd = weather.get("wind_dir", "")
    if ws >= 5:     sc_text += f" {wd}{ws}mの影響大。"
    if weather.get("wave", 0) >= 8: sc_text += " 高波注意。"
    ...
    s2 = []
    if pat == "逃げ":
        if by_course.get(2): s2.append((2, 0.343))
        if by_course.get(3): s2.append((3, 0.271))
    elif pat == "差し":
        if by_course.get(1): s2.append((1, 0.60))
    elif pat == "まくり差し":
        if by_course.get(1): s2.append((1, 0.55))
    elif pat == "まくり":
        outer = min(top["course"] + 1, 6)
        if by_course.get(outer): s2.append((outer, 0.40))
        if by_course.get(1): s2.append((1, 0.30))
    ...
    if gap < 1.0:
        ...
        fc2, sc2 = top["course"], scored_racers[1]["course"]
        fms = [f"{fc2}-{sc2}-全", f"{sc2}-{fc2}-全"]
        btype = "3連単(穴型)"
    else:
        rtype = "買い"
        ...
        fc2 = top["course"]
        if s2:
            fms = [f"{fc2}-{'/'.join(str(x[0]) for x in s2[:2])}-全"]
        else:
            fms = [f"{fc2}-{scored_racers[1]['course']}-全"]
        btype = "3連単(基本型)"

    bets = []
    if rtype != "見送り":
        for s2c, s2r in s2[:2]:
            for tr in scored_racers[:4]:
                tc = tr["course"]
                if tc != top["course"] and tc != s2c:
                    hit = fr * s2r * 0.25
                    bets.append({
                        "bet": f"{top['course']}-{s2c}-{tc}",
                        "hit_rate": round(hit, 4),
                        "req_odds": round(1.0 / hit, 1) if hit > 0 else 999,
                        "note": "本命" if hit > 0.05 else "押さえ",
                    })
...
def main():
    ...
    if analysis.get("bets"):
        ...
        bdf = pd.DataFrame([{
            "買い目": b["bet"],
            "分類": b["note"],
            "的中率": f'{b["hit_rate"]*100:.1f}%',
            "必要倍率": f'{b["req_odds"]}倍以上',
